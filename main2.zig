...
pub fn merge_sort(alloc: Allocator, src: []i64) error{OutOfMemory}!void {
    if (src.len == 0) return;
    // Continuations needed: ~2n (one MergeSecond + one DoMerge per element)
    // MemoryPool allocates in pages, ensure minimum size for pool overhead
    const min_buffer = 64 * 1024; // 64KB minimum for MemoryPool pages
    const needed = @sizeOf(continuation) * src.len * 3;
    const buffer_size = @max(min_buffer, needed);
    const buffer: []u8 = try alloc.alloc(u8, buffer_size);
    var fixed_buffer = std.heap.FixedBufferAllocator.init(buffer);
    const fixed_buffer_alloc = fixed_buffer.allocator();
    var pool = Pool.init(fixed_buffer_alloc);
    ...
    alloc.free(scratch);
    pool.deinit(); // free all continuations all at once for free
    alloc.free(buffer);
}
